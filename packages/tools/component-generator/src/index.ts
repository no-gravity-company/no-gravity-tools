...
`
  ...
  const main = async (): Promise<void> => {
    ...
    if (componentNameIndex > -1) {
        ...
        // Component .tsx file
        const componentFilePath = path.join(componentFolderPath, `${componentNameValue}.tsx`);
        const interpolation = getComponentTemplateWithValues(componentNameValue);
        await fsp.writeFile(componentFilePath, interpolation, { encoding: 'utf-8' });
        // .gitignore
        const gitIgnorePath = path.join(componentFolderPath, '.gitignore');
        const gitIgnoreTemplate = getGitIgnoreTemplate();
        await fsp.writeFile(gitIgnorePath, gitIgnoreTemplate, { encoding: 'utf-8' });
        // package.json
        ...
        await fsp.writeFile(packageJsonPath, packageJsonTemplate, { encoding: 'utf-8' });
        // types
